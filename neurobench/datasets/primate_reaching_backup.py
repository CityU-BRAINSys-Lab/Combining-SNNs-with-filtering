...
class PrimateReaching(NeuroBenchDataset):
    ...
    def split_data(self):
        ...
        split_num = 1 # Change to 1 for now.
        ...
        # total_segments = self.time_segments.shape[0]
        # train_len = math.floor(self.train_ratio * total_segments)
        # val_len = math.floor(total_segments * ((1 - self.train_ratio) / 2))
        # test_len = total_segments - train_len - val_len

        # for i in range(total_segments):
        #     if i < train_len:
        #         self.ind_train += list(np.arange(self.time_segments[i, 0], self.time_segments[i, 1]))
        #     elif i >= train_len and i < train_len + val_len:
        #         self.ind_val += list(np.arange(self.time_segments[i, 0], self.time_segments[i, 1]))
        #     else:
        #         self.ind_test += list(np.arange(self.time_segments[i, 0], self.time_segments[i, 1]))
