...
class SNNModelStreaming(nn.Module):
    ...
    def forward(self, x):
        predictions = []
        block_filter_last, block_filter_middle, full_filtfilt = False, False, False
        win_size = 32
        ...
        for seq in range(seq_length):
            ...
            pred, input_, fc1_output, fc2_output = self.single_forward(spikes)
            U_x = self.v_x*pred[:, 0]
            U_y = self.v_y*pred[:, 1]
